...
export default function AuthPage() {
  ...
  const onSubmit = async (data) => {
    setIsLoading(true);
    setAuthError('');
    console.log('Submitted Auth Data:', data);
    
    // Simulate API Auth Request
    try {
      await new Promise(resolve => setTimeout(resolve, 1500));
      
      // Save login state to localStorage so the global Header reflects profile avatar
      localStorage.setItem('isLoggedIn', 'true');
      const nameVal = data.fullName || data.email.split('@')[0] || 'Alex Chen';
      localStorage.setItem('userName', nameVal);
      
      // Dynamic routing after successful auth
      if (isRegister) {
        navigate('/assessment');
      } else {
        navigate('/result'); // /result is the actual dashboard page
      }
    } catch (error) {
      console.error('Auth Error:', error);
      setAuthError('Kredensial salah. Silakan coba lagi.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        >
          ...
          <div className="position-relative z-3 px-4 mb-auto mt-auto max-w-[480px]">
            ...
            <p className="fs-5 text-white/90 leading-relaxed">
              Join the community of bio-hackers using clinical data to optimize every espresso, matcha, and cold brew for peak performance.
            </p>
          </div>
        ...
        <div className="flex-1 min-h-[calc(100vh-73px)] flex flex-col justify-center items-center py-4 px-4 sm:px-6 lg:px-8 bg-[#fcf9f8] relative">
          ...
          <div className="w-full max-w-[440px] bg-white border border-[#d4c3ba] p-4 sm:p-5 rounded-4 shadow-sm relative z-1">
            ...
            <div className="d-flex align-items-center mb-4">
              ...
              <span className="px-3 text-[#82746d] fw-semibold uppercase tracking-widest" style={{ fontSize: '10px' }}>OR EMAIL</span>
              <div className="flex-grow-1 border-top border-[#d4c3ba]"></div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="d-flex flex-column gap-3">
              ...
              {isRegister && (
                <div>
                  <label className="form-label text-dark fw-medium mb-1.5" style={{ fontSize: '13px' }}>Nama Lengkap</label>
                  <div className="position-relative">
                    ...
                    <input
                      ...
                      style={{ fontSize: '15px' }}
                      {...register("fullName", { 
                        required: isRegister ? "Nama lengkap wajib diisi" : false,
                        minLength: { value: 3, message: "Minimal 3 karakter" }
                      })}
                    />
                  </div>
                  {errors.fullName && <span className="text-danger text-xs mt-1 d-block">{errors.fullName.message}</span>}
                </div>
              )}

              {/* Input Email */}
              <div>
                <label className="form-label text-dark fw-medium mb-1.5" style={{ fontSize: '13px' }}>Email Kantor / Pribadi</label>
                <div className="position-relative">
                  <span className="position-absolute start-0 top-50 translate-middle-y ps-3 text-muted">
                    <Mail size={18} />
                  </span>
                ...
                {errors.email && <span className="text-danger text-xs mt-1 d-block">{errors.email.message}</span>}
              </div>

              {/* Input Password */}
              <div>
                <div className="d-flex justify-content-between align-items-center mb-1.5">
                  <label className="form-label text-dark fw-medium m-0" style={{ fontSize: '13px' }}>Password</label>
                  {!isRegister && (
                    <button 
                      type="button"
                      onClick={() => alert('Fitur pemulihan password akan segera hadir.')} 
                ...
                {errors.password && <span className="text-danger text-xs mt-1 d-block">{errors.password.message}</span>}
              </div>

              {/* Input Confirm Password (Only in Register mode) */}
              {isRegister && (
                <div>
                  <label className="form-label text-dark fw-medium mb-1.5" style={{ fontSize: '13px' }}>Konfirmasi Password</label>
                  <div className="position-relative">
                    <span className="position-absolute start-0 top-50 translate-middle-y ps-3 text-muted">
                      <Lock size={18} />
                    </span>
                    <input
                      ...
                      {...register("confirmPassword", { 
                        required: isRegister ? "Konfirmasi password wajib diisi" : false,
                        validate: value => value === passwordVal || "Password tidak cocok"
                      })}
                    />
                    <button
                      type="button"
                  ...
                  {errors.confirmPassword && <span className="text-danger text-xs mt-1 d-block">{errors.confirmPassword.message}</span>}
                </div>
              )}
              ...
              <div className="form-check d-flex align-items-start mt-2">
                <input
                  ...
                  {...register("rememberOrAgree", { 
                    required: isRegister ? "Anda harus menyetujui ketentuan layanan kami" : false 
                  })}
                />
                <label htmlFor="rememberOrAgree" className="form-check-label ps-2 text-[#50453e] cursor-pointer" style={{ fontSize: '13.5px', lineHeight: '1.4' }}>
                  {isRegister 
                    ? <span>Saya menyetujui <a href="#terms" className="text-[#1b6d24] fw-semibold hover:underline">Ketentuan Layanan</a> & <a href="#privacy" className="text-[#1b6d24] fw-semibold hover:underline">Kebijakan Privasi</a></span>
                    : 'Tetap login selama 30 hari'
                  }
                </label>
              </div>
              {errors.rememberOrAgree && <span className="text-danger text-xs d-block">{errors.rememberOrAgree.message}</span>}
            ...
            >
              <div className="bg-[#1b6d24] shrink-0 rounded-circle animate-pulse" style={{ width: '8px', height: '8px' }} />
              <div className="text-start">
                <span className="text-[#1b6d24] fw-semibold tracking-wider uppercase block" style={{ fontSize: '10px' }}>
