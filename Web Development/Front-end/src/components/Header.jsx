...
import { User, LogOut, Settings, ChevronDown, Menu, X, ArrowUpRight, HelpCircle, Info, Heart } from 'lucide-react';
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const name = localStorage.getItem('userName') || 'Alex Chen';
    setIsLoggedIn(loggedIn);
    setUserName(name);
  }, [location.pathname]); // sync on navigation/route changes
  ...
  return (
    <header className="bg-black text-white sticky-top z-50 shadow-md py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
      ...
      <div className="container-xl d-flex justify-content-between align-items-center px-4 position-relative">
        ...
        <nav className="d-none d-md-flex align-items-center gap-4">
          ...
          {isLoggedIn ? (
            ...
            <>
              <button onClick={() => handleNavClick('features')} className="bg-transparent border-0 text-white text-decoration-none opacity-75 hover:opacity-100 transition-colors py-2">
                Fitur
              </button>
              <button onClick={() => handleNavClick('how-it-works')} className="bg-transparent border-0 text-white text-decoration-none opacity-75 hover:opacity-100 transition-colors py-2">
                Cara Kerja
              </button>
              <button onClick={() => handleNavClick('analysis')} className="bg-transparent border-0 text-white text-decoration-none opacity-75 hover:opacity-100 transition-colors py-2">
                Preview Dashboard
              </button>
              <button onClick={() => handleNavClick('about')} className="bg-transparent border-0 text-white text-decoration-none opacity-75 hover:opacity-100 transition-colors py-2">
                Tentang Kami
              </button>
            </>
          )}
            ...
            <div className="position-relative" ref={dropdownRef}>
              ...
              >
                <div style={{
                  ...
                  fontSize: '15px', color: '#fff', fontWeight: '700', border: '2px solid rgba(255,255,255,0.2)'
                }}>
              ...
              {isProfileDropdownOpen && (
                <div className="position-absolute end-0 mt-2 bg-white rounded-3 shadow-lg border border-[#d4c3ba] py-2 overflow-hidden animate-fade-in" style={{ width: '260px', zIndex: 1000, top: '100%' }}>
                  ...
                  <div className="py-1">
                    ...
                    <button 
                      onClick={() => { setIsProfileDropdownOpen(false); handleNavClick('features'); }}
                    ...
                    <button 
                      onClick={() => { setIsProfileDropdownOpen(false); handleNavClick('how-it-works'); }}
                      className="custom-profile-dropdown-item d-flex align-items-center gap-2.5 px-4 py-2.5 w-100 bg-transparent border-0"
                      style={{ fontSize: '14px' }}
                    >
                    ...
                    <button 
                      onClick={() => { setIsProfileDropdownOpen(false); handleNavClick('about'); }}
                      className="custom-profile-dropdown-item d-flex align-items-center gap-2.5 px-4 py-2.5 w-100 bg-transparent border-0"
                      style={{ fontSize: '14px' }}
                    >
        ...
        {/* MOBILE DRAWER */}
        {isMobileMenuOpen && (
          <div className="d-md-none bg-black border-top border-secondary border-opacity-20 position-absolute start-0 end-0 py-3 px-4 shadow-lg" style={{ top: '100%', zIndex: 40 }}>
            <nav className="d-flex flex-column gap-2">
              ...
              {isLoggedIn ? (
                ...
                <>
                  <button onClick={() => handleNavClick('features')} className="bg-transparent border-0 text-white text-decoration-none opacity-80 hover:opacity-100 text-start py-2 border-bottom border-secondary border-opacity-25">
                    Fitur
                  </button>
                  <button onClick={() => handleNavClick('how-it-works')} className="bg-transparent border-0 text-white text-decoration-none opacity-80 hover:opacity-100 text-start py-2 border-bottom border-secondary border-opacity-25">
                    Cara Kerja
                  </button>
                  <button onClick={() => handleNavClick('analysis')} className="bg-transparent border-0 text-white text-decoration-none opacity-80 hover:opacity-100 text-start py-2 border-bottom border-secondary border-opacity-25">
                    Preview Dashboard
                  </button>
                  <button onClick={() => handleNavClick('about')} className="bg-transparent border-0 text-white text-decoration-none opacity-80 hover:opacity-100 text-start py-2 mb-2">
                    Tentang Kami
                  </button>
                  <button 
                    onClick={() => { setIsMobileMenuOpen(false); navigate('/assessment'); }} 
